...
public class UniqueCons{
    public static void main(String[] args) throws Exception {new UniqueCons().run();}
    ...
    void solve() throws Exception {
        ...
        while(t-->0){
            ...
            helper(n,k,1,new ArrayList<>(),set);
            List<List<Integer>>ans = new ArrayList<>();
            for(List<Integer> l:set){
                if(isValid(l))ans.add(l);
                //out.println(l);
            }
    ...
    void helper(int n,int k,int i,List<Integer>l,Set<List<Integer>> set){
        if(i>n){
            if(l.size()==k ){
    ...
    boolean isValid(List<Integer> l){
        int n = l.size();

        for(int j=1;j<n;j++){
            if((l.get(j)%(l.get(j-1)))!=0) return false;
        }
        return true;
    }
    void formDP(){
