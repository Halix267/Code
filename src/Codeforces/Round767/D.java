...
public class D {
        ...
        while(t-->0){
           ...
           boolean flag = false;
            ...
            for(int i=0;i<n;i++){
                ...
                String tmp = sc.reverse().toString();

                if(set.contains(tmp)|| set.contains(tmp.substring(0,2))){
                    flag = true;
                    break;
                }
                set.add(a[i]);
                set.add(a[i].substring(0,2));
            }
    ...
    int findMissng(List<Integer> l,int start,int end){
        if (start > end)
            return end + 1;

        if (start != l.get(start))
            return start;
        ...
        if (l.get(mid) == mid)
            return findMissng(l, mid+1, end);

        return findMissng(l, start, mid);
    }
