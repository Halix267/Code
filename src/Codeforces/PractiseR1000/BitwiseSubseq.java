...
public class BitwiseSubseq {
    public static void main(String[] args) throws Exception {new BitwiseSubseq().run();}
        ...
        while(t-->0){
            ...
            for(int i=1;i<n;i++){
                ...
                for(int j=0;j<i;j++){
                    int tmp1= a[i]&a[j];
                    int tmp2 = a[i]|a[j];
                    out.println(tmp1+" "+tmp2+" "+i+" "+j);
                    if((tmp1)*2<(tmp2) && dp[i]<dp[j]+1){
                        dp[i] = dp[j]+1;
                    }
                }
            }
